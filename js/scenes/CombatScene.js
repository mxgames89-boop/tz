...
export class CombatScene {
    ...
    update(deltaTime){
        ...
        if (window.entities) window.entities.forEach(entity => entity.update(deltaTime));

        if(window.gameState === 'PLANNING'){
            this.turnManager.updatePlanning?.(deltaTime);
        }

        if(window.gameState === 'EXECUTION'){
            this.turnManager.updateExecution(deltaTime);
            this.objectmap.objects.forEach(obj => {if(obj.update) this.objectmap.update(deltaTime);});
        }
    }
